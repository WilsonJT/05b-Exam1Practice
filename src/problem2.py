...
def problem2b(rect, n, delta, win):
    ...
    if n < 0:
        n = -n
    rect.attach_to(win)
    down_left = rect.get_lower_left_corner()
    up_right = rect.get_upper_right_corner()
    for i in range(n):
        x1 = down_left.x - delta * i
        x2 = up_right.x + delta * i
        y1 = down_left.y + delta * i
        y2 = up_right.y - delta * i
        p1 = rg.Point(x1, y1)
        p2 = rg.Point(x2, y2)
